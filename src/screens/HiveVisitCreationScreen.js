...
const HiveVisitCreationScreen = ({ route }) => {
  ...
  const handleSubmit = async () => {
    ...
    // TODO not rendering
    let parsedCovata =
      covata && !isNaN(parseFloat(covata)) ? parseFloat(covata) : 0;
  ...
  return (
    <View style={styles.container}>
      <Button
        mode="contained"
        onPress={() =>
          navigation.navigate("HiveDetailScreen", { hiveId: hiveId })
        }
      >
        Visualizza lo storico visite
      </Button>
      ...
      <ScrollView contentContainerStyle={styles.contentContainer}>
        ...
        <List.Item
          ...
          right={() => (
            <Switch value={cupoliniReali} onValueChange={setCupoliniReali} />
          )}
        />
        <List.Item
          title="Celle Reali"
          right={() => (
            <Switch value={celleReali} onValueChange={setCelleReali} />
          )}
        />
        <TextInput
          label="Notes"
